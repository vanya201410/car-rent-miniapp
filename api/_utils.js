...
export function buildAdminBookingText({ title, booking, car }) {
  ...
  return [
    ...
    booking.rental_payment_status === 'paid' ? `<b>Аренда оплачена:</b> ${escapeHtml(booking.rental_paid_amount || booking.total_price || 0)} € ✅` : '',
    booking.rental_payment_status === 'partial' ? `<b>Аренда оплачена частично:</b> ${escapeHtml(booking.rental_paid_amount || booking.prepayment_amount || 0)} €` : '',
    booking.full_payment_status === 'paid' ? '<b>Тип оплаты:</b> полная оплата аренды онлайн ✅' : '',
    booking.remaining_payment_status === 'paid' ? `<b>Остаток оплачен онлайн:</b> ${escapeHtml(booking.remaining_paid_amount || 0)} € ✅` : '',
    ...
    booking.pre_rental_photos_status ? `<b>Фото до выдачи:</b> ${escapeHtml(booking.pre_rental_photos_status)}` : '',
    booking.post_rental_photos_status ? `<b>Фото после возврата:</b> ${escapeHtml(booking.post_rental_photos_status)}` : '',
    car?.deposit ? `<b>Залог:</b> ${escapeHtml(car.deposit)} €` : '',
    booking.deposit_status ? `<b>Статус залога:</b> ${escapeHtml(booking.deposit_status)}` : '',
    booking.deposit_held_amount ? `<b>Удержано из залога:</b> ${escapeHtml(booking.deposit_held_amount)} €${booking.deposit_hold_reason ? ' · ' + escapeHtml(booking.deposit_hold_reason) : ''}` : '',
    booking.return_status ? `<b>Возврат авто:</b> ${escapeHtml(booking.return_status)}` : '',
    booking.has_pending_charges ? '<b>Доп. начисления:</b> есть неоплаченные штрафы/расходы ⚠️' : '',
